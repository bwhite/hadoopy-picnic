...
def do_GET(self):
  ...
  #cv.PutText(im,s,(80,100),cv.InitFont(cv.CV_FONT_HERSHEY_SIMPLEX,0.5,0.5), (255,255,0))
  
  with get_temp_filename('.jpg') as filename:
    im = np.zeros((256,256,3),'u1')
    #Image.open('tiler/%d_%d_%d.jpg' % (z, x, y)).save()
    shutil.copy('tiler/%d_%d_%d.jpg' % (z, x, y), filename)
